          ...
          {branches.length === 0 ? (
            ...
            <div className="space-y-3">
              {branches.map((branch) => (
                <div
                  key={branch.id}
                  className="flex items-center justify-between p-3 rounded-xl bg-gym-950/60"
                >
                  <div>
                    <p className="text-sm font-semibold text-slate-100">
                      {branch.name}
                      {branch.code && (
                        <span className="ml-2 px-1.5 py-0.5 rounded bg-gym-950 font-mono text-[10px] text-brand-cyan">
                          {branch.code}
                        </span>
                      )}
                    </p>
      ...
      {tab === 'staff' && (
        <Card className="p-6 space-y-5">
          ...
          {!isAdmin ? (
            ...
            <form onSubmit={handleCreateStaff} className="space-y-4 p-4 rounded-xl bg-gym-950/60">
              {staffError && (
              ...
              >
                ...
                {isAdmin && account.id !== user?.id ? (
                  <select
                    ...
                    className="rounded-lg bg-gym-850 text-xs text-slate-200 px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-brand-gold/40"
                  >
                    {ROLES.map((role) => (
                      <option key={role} value={role}>
                        {role}
